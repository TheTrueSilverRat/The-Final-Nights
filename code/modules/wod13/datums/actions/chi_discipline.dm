...
/datum/action/chi_discipline/proc/register_to_availability_signals()
	//this should only go through if it's the first Discipline gained by the mob
	for (var/datum/action/action in owner.actions)
		if (action == src)
			continue
		if (istype(action, /datum/action/chi_discipline))
			return
...
/datum/action/chi_discipline/Trigger()
	...
	for (var/datum/action/action in owner.actions)
		if (istype(action, /datum/action/chi_discipline))
			var/datum/action/chi_discipline/other_chi_discipline = action
			other_chi_discipline.end_targeting()
...
/datum/action/chi_discipline/proc/end_targeting()
	...
	targeting = FALSE
	//this goes to a specific mob define var, don't change it to chi_discipline
	owner.discipline_targeting = FALSE
	client.mouse_pointer_icon = initial(client.mouse_pointer_icon)

/datum/action/chi_discipline/proc/handle_click(mob/source, atom/target, click_parameters)
	SIGNAL_HANDLER
...
/datum/action/chi_discipline/proc/begin_targeting()
	var/client/client = owner?.client
	if (!client)
		return
	if (targeting)
