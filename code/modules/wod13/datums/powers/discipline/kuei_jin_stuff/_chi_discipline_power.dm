/datum/chi_discipline_power
	/// Name of the Discipline power
	var/name = "Chi Discipline power name"
	/// Description of the Discipline power
	var/desc = "Chi Discipline power description"
...
/datum/chi_discipline_power/New(datum/chi_discipline/chi_discipline)
	if(!chi_discipline)
		CRASH("discipline_power [src.name] created without a parent chi discipline!")

	src.chi_discipline = chi_discipline
	src.owner = chi_discipline.owner
 ...
 * cooldown.
 */
/datum/chi_discipline_power/proc/get_cooldown()
 ...
 * means not active.
 */
/datum/chi_discipline_power/proc/get_duration()
 ...
 */
/datum/chi_discipline_power/proc/can_afford()
 ...
 */
/datum/chi_discipline_power/proc/can_activate_untargeted(alert = FALSE)
	...
	if (islist(grouped_powers))
		for (var/exclude_power in grouped_powers)
			var/datum/chi_discipline_power/found_power = chi_discipline.get_power(exclude_power)
	...
	//the user cannot afford the power's total chi expenditure
	if (!can_afford())
		if (alert)
			to_chat(owner, span_warning("You do not have the required chi to cast [src]!"))
		return FALSE
 ...
 * * alert - if this is being checked by the user and should give feedback on why it can't activate.
 */
/datum/chi_discipline_power/proc/can_activate(atom/target, alert = FALSE)
	SHOULD_CALL_PARENT(TRUE)
...
/**
 * Spends necessary resources (chi) and makes sure activation is valid
 ...
 * * target - what the targeted Discipline (null otherwise) is being used on.
 */
/datum/chi_discipline_power/proc/pre_activation(atom/target)
 ...
 * * target - what the targeted Discipline (null otherwise) is being used on.
 */
/datum/chi_discipline_power/proc/pre_activation_checks(atom/target)
	return TRUE
 ...
 */
/datum/chi_discipline_power/proc/do_activate_sound()
	if (activate_sound)
 ...
 */
/datum/chi_discipline_power/proc/do_effect_sound(atom/target)
	if (effect_sound)
 ...
 * it's used on.
 */
/datum/chi_discipline_power/proc/do_npc_aggro(atom/target)
 ...
 */
/datum/chi_discipline_power/proc/do_masquerade_violation(atom/target)
...
/**
 * Overridable proc handling the spending of resources (Chi)
 * when casting the power. Returns TRUE if successfully spent,
 * returns FALSE otherwise.
 */
/datum/chi_discipline_power/proc/spend_resources()
	if (can_afford())
 ...
 * the power.
 */
/datum/chi_discipline_power/proc/do_caster_notification(target)
 ...
 */
/datum/chi_discipline_power/proc/do_logging(target)
	log_combat(owner, target ? target : owner, "casted the power [src.name] of the Discipline [chi_discipline.name] on")
 ...
 */
/datum/chi_discipline_power/proc/do_duration(atom/target)
 ...
 */
/datum/chi_discipline_power/proc/do_cooldown(on_activation = FALSE)
 ...
 * * target - what the targeted Discipline (null otherwise) is being used on.
 */
/datum/chi_discipline_power/proc/try_activate(atom/target)
 ...
 */
/datum/chi_discipline_power/proc/duration_expire(atom/target)
 ...
 */
/datum/chi_discipline_power/proc/cooldown_expire()
	owner.update_action_buttons()
 ...
 */
/datum/chi_discipline_power/proc/can_deactivate_untargeted()
	SHOULD_CALL_PARENT(TRUE)
 ...
 * * target - what the targeted Discipline (null otherwise) is being used on.
 */
/datum/chi_discipline_power/proc/can_deactivate(atom/target)
	SHOULD_CALL_PARENT(TRUE)
 ...
 */
/datum/chi_discipline_power/proc/deactivate(atom/target, direct = FALSE)
	SHOULD_CALL_PARENT(TRUE)
 ...
 */
/datum/chi_discipline_power/proc/try_deactivate(atom/target, direct = FALSE, alert = FALSE)
	SHOULD_NOT_OVERRIDE(TRUE)
 ...
 */
/datum/chi_discipline_power/proc/post_gain()
	return
 ...
 * * target - what the targeted Discipline (null otherwise) is being used on.
 */
/datum/chi_discipline_power/proc/refresh(atom/target)
	if (!active)
		return
	if (!owner)
		return
	...
	if (spend_resources())
		if(cost_yang > 0)
			to_chat(owner, span_warning("[src] consumes your yang chi to stay active."))
			if(cost_yin > 0)
				to_chat(owner, span_warning("[src] consumes your yin chi to stay active."))
				if(cost_demon > 0)
					to_chat(owner, span_warning("[src] consumes your demon chi to stay active."))
		if (!duration_override)
			do_duration(target)
	else
 ...
 */
/datum/chi_discipline_power/proc/do_refresh_checks(atom/target)
	return TRUE
 ...
 */
/datum/chi_discipline_power/proc/clear_duration_timer(to_clear = 1)
	if (toggled && (duration_length == 0))
		return
